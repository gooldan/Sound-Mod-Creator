...
Rectangle {
    ...
    MyRoundCloseButton {
        anchors.right: myBut.visible ? myBut.left : parent.right
        anchors.rightMargin: 3
        anchors.top: parent.top
        anchors.topMargin: 3
        id: myButDown
    ...
    function filter(text) {
        ...
        for (var i = 0; i < model.count; ++i) {
            ...
            if (model.get(i)[neededName].toLowerCase().indexOf(text.toLowerCase(
                                                                   )) >= 0) {
                indexes.push(i)
        ...
        var currentIndex = 0
        if (indexes.length > 0) {
            listViewBase.positionViewAtIndex(indexes[currentIndex],
                                             ListView.Center)
